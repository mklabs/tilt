...
class Tilt {
  constructor(options) {
    ...
    this.asset = new Asset(this.options.assets || {});
  ...
  assets(pattern) {
    this.asset.assetsPattern = pattern;
    return this;
  }
  ...
  createServer() {
    ...
    var staticDirs = glob.sync(this.staticPattern);
    debug('Static dirs', staticDirs);

    staticDirs = staticDirs.map((dir) => {
      return {
        dir: dir,
    ...
    return http.createServer((req, res) => {
      ...
      if (/^\/assets\//.test(req.url)) return this.asset.handle(req, res, done);
      ...
      var middlewares = staticDirs.concat();

      (function nextDir(serveDir) {
        if (!serveDir) return next.call(this);

        debug('Init static middleware for %s', serveDir.dir);
        serveDir.middleware(req, res, (err) => {
          if (err) return this.error(err);
          nextDir.call(this, middlewares.shift());
        });
      }.bind(this))(middlewares.shift());
